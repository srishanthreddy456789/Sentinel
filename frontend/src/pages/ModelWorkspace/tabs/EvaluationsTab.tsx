...
export const EvaluationsTab: React.FC = () => {
  ...
  const runs: EvaluationRun[] = evaluationsMap[selectedModel.id] || [];
  ...
  const avgPassRate = totalTestCases ? Number(((totalPassed / totalTestCases) * 100).toFixed(1)) : 100.0;
      ...
      <div className="sentinel-card overflow-hidden">
        ...
        {runs.length > 0 ? (
          ...
          <div className="p-8 text-center text-zinc-400 space-y-2">
            <ClipboardCheck className="w-8 h-8 text-emerald-400 mx-auto" />
            <p className="text-xs text-white font-medium">No Evaluation Runs Recorded</p>
            <p className="text-xs text-zinc-500">
              Run evaluation tests or ingest requests to trigger quality checks for {selectedModel.name}.
            </p>
          </div>
        )}
      </div>
