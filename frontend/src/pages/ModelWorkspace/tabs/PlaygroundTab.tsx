...
export const PlaygroundTab: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      // 1. Send request to backend ingestion & evaluation pipeline
      ...
      // 2. Direct client execution fallback when backend is unavailable or offline
      try {
        let outputText = '';
        ...
        if (
          ...
          if (apiKeyClean) {
            ...
            if (res.ok) {
              ...
              outputText =
                ...
                `Response generated by ${selectedModel.name}.`;
            }
          }
        } else if (
          ...
          try {
            ...
            if (res.ok) {
              const resData = await res.json();
              outputText = resData.message?.content || `Local Ollama model response.`;
            }
          } catch (e) {
            console.warn('Ollama playground call:', e);
          }
        }

        if (!outputText) {
          outputText = `Output from ${selectedModel.name} (${selectedModel.provider}): Evaluated prompt against SENTINEL quality gate metrics.`;
        }
        ...
        const latencyMs = Date.now() - startTime;
        setResponse(`Output from ${selectedModel.name}: Execution evaluated.`);
        setEvalResult({
